...
#[tokio::test]
async fn operator_grant_http_resolves_owner_from_canonical_runner_inventory() {
    let (_dir, db, _) = fixture();
    let token = "live-user-token";
    let token_hash = hash_token(token);
    db.insert_api_key(
        &ApiKeyRecord {
            id: "live-key".into(),
            user_id: "user".into(),
            name: "Live Desktop".into(),
            key_prefix: "live".into(),
            created_at: 1,
            last_used_at: None,
            revoked_at: None,
            scopes: "runtime:read runner:manage".into(),
            expires_at: None,
            kind: TOKEN_KIND_USER.into(),
            allowed_client_id: None,
        },
    ...
    let registry = std::sync::Arc::new(crate::runner_http::RunnerRegistry::default());
    registry
        .register(crate::test_support::current_runner_registration(
            crate::runner_protocol::RunnerRegisterRequest {
                ...
                client_id: "mini".into(),
                runner_instance_id: "inst-mini".into(),
                runner_protocol_generation: crate::runner_protocol::RUNNER_PROTOCOL_GENERATION_V2,
                display_name: Some("mini".into()),
                owner: Some("alice".into()),
                hostname: None,
                host_context: None,
                capabilities: crate::runner_protocol::RunnerCapabilities::default(),
                policy: None,
            },
        ))
        .await
    ...
    let runtime = std::sync::Arc::new(
        crate::tool_runtime::ToolRuntime::new_for_tests_with_runner_registry(registry),
    );
    ...
    let mut response = TestClient::post("http://localhost/grant")
        .json(&json!({"client_id":"mini", "user_token_hash":token_hash}))
        .send(&service)
        .await;
    assert_eq!(response.status_code.unwrap(), StatusCode::OK);
    ...
    let stored = db.get_api_key_by_hash(&hash_token(token)).unwrap().unwrap();
